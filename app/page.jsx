...
export default function Home() {
  ...
  useEffect(() => {
    async function loadLibrary() {
      ...
      if (data) setLibrary(await Promise.all(data.map(async item => { const savedStory = { ...item.story_json, id: item.id, title: item.title, language: item.language, cloud: true }; if (savedStory.illustrationPath) { const { data: signed } = await supabase.storage.from('story-media').createSignedUrl(savedStory.illustrationPath, 3600); savedStory.illustrationUrl = signed?.signedUrl || ''; } return savedStory; })));
    }
    loadLibrary();
  }, []);
  ...
  async function saveStory() {
    ...
    if (illustrationUrl?.startsWith('data:image/')) {
      const mediaPath = `${user.id}/${crypto.randomUUID()}.webp`;
      const imageBlob = await fetch(illustrationUrl).then(response => response.blob());
      const { error: uploadError } = await supabase.storage.from('story-media').upload(mediaPath, imageBlob, { contentType: 'image/webp', upsert: false });
      if (uploadError) { setStatus('Could not save the story illustration.'); return; }
      storyToSave.illustrationPath = mediaPath;
    }
    delete storyToSave.illustrationUrl;
    const { data, error } = await supabase.from('stories').insert({ user_id: user.id, title: story.title, language: story.language || 'English', status: 'ready', story_json: storyToSave }).select('id').single();
    if (error) { setStatus('Could not save to your parent library.'); return; }
    setLibrary(current => [{ ...story, ...storyToSave, id: data.id, cloud: true }, ...current.filter(item => item.title !== story.title)].slice(0, 30));
    setStatus('Saved to your cloud library.');
  }

  async function removeStory(title, id) { if (id) { const supabase = createClient(); const storyToRemove = library.find(item => item.id === id); if (storyToRemove?.illustrationPath) await supabase.storage.from('story-media').remove([storyToRemove.illustrationPath]); await supabase.from('stories').delete().eq('id', id); } setLibrary(current => current.filter(item => item.title !== title)); }
