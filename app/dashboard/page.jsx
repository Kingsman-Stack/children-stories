...
export default function Dashboard() {
  ...
  const [characterTraits, setCharacterTraits] = useState('curious, kind');
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: profiles }, { data: savedStories }, { data: consent }, { data: savedCharacters }] = await Promise.all([
        ...
        supabase.from('characters').select('id,name,traits,created_at').order('created_at', { ascending: true }),
      ]);
  ...
  async function addCharacter(event) { event.preventDefault(); if (!characterName.trim() || !user) return; const { data, error } = await createClient().from('characters').insert({ user_id: user.id, name: characterName.trim(), traits: characterTraits.trim() }).select('id,name,traits,created_at').single(); if (error) { setMessage(error.message); return; } setCharacters(current => [...current, data]); setCharacterName(''); setMessage('Recurring character added.'); }
  async function deleteCharacter(id) { await createClient().from('characters').delete().eq('id', id); setCharacters(current => current.filter(character => character.id !== id)); }
  ...
  return <main className={`${styles.page} ${darkTheme ? overrides.darkPage : ''}`}>
    <nav className={styles.nav}><a className={styles.brand} href="/">story<span>sprout</span></a><div><button className={styles.themeButton} type="button" onClick={toggleTheme}>{darkTheme ? 'Light theme' : 'Dark theme'}</button><button className={styles.signout} type="button" onClick={signOut}>Sign out</button></div></nav>
